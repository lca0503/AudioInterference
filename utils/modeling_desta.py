...
from transformers import (AutoConfig, AutoModelForCausalLM, AutoProcessor,
                          AutoTokenizer, BertConfig, PretrainedConfig,
                          PreTrainedModel, WhisperForConditionalGeneration)
...
class DestaModel(PreTrainedModel):
    ...
    def chat(self, messages, prefix="", max_new_tokens=1024, do_sample=False, temperature=None, top_p=None, top_k=None, text_only=False):
        """
        ...
        """
        if text_only:
            inputs_embeds, attention_mask = self.prepare_text_only_llm_input(messages, prefix)
        else:
            ...
            inputs, audio_position = self.process_text(messages, prefix, audio_path, transcription)
    ...
    def prepare_text_only_llm_input(self, messages, prefix):
        context = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True) + prefix
        inputs = self.tokenizer(context, return_tensors="pt")

        input_ids = inputs.input_ids.to(self.llama.device)
        attention_mask = inputs.attention_mask.to(self.llama.device)
    ...
    def process_text(self, messages, prefix, audio_path, transcription):
        context = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True) + prefix
